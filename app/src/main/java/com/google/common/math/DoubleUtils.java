...
final class DoubleUtils {
    ...
    static double bigToDouble(BigInteger x) {
        ...
        if ((1 & twiceSignifFloor) != 0) {
            increment = (1 & signifFloor) != 0 || absX.getLowestSetBit() < shift;
        } else {
            increment = false;
        }
        return Double.longBitsToDouble(((((long) (exponent + EXPONENT_BIAS)) << 52) + (increment ? signifFloor + 1 : signifFloor)) | (((long) x.signum()) & SIGN_MASK));
    }
