...
public class CollapsingToolbarLayout extends FrameLayout {
    ...
    private boolean isToolbarChild(View child) {
        if (this.mToolbarDirectChild == null || this.mToolbarDirectChild == this) {
            return child == this.mToolbar;
        }
        if (child != this.mToolbarDirectChild) {
            return false;
        }
        return true;
    }
    ...
    public void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (this.mCollapsingTitleEnabled && this.mDummyView != null) {
            if (ViewCompat.isAttachedToWindow(this.mDummyView)) {
                z = this.mDummyView.getVisibility() == 0;
            } else {
                z = false;
            }
    ...
    public void setScrimsShown(boolean shown) {
        setScrimsShown(shown, ViewCompat.isLaidOut(this) ? !isInEditMode() : false);
    }
