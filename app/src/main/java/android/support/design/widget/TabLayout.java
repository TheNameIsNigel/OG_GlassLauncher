...
public class TabLayout extends HorizontalScrollView {
    ...
    private int calculateScrollXForTab(int position, float positionOffset) {
        ...
        if (position + 1 < this.mTabStrip.getChildCount()) {
            view = this.mTabStrip.getChildAt(position + 1);
        } else {
            view = null;
        }
        int selectedWidth = selectedChild != null ? selectedChild.getWidth() : 0;
        int nextWidth = view != null ? view.getWidth() : 0;
    ...
    class TabView extends LinearLayout {
        ...
        public final void update() {
            ...
            if (customView != null) {
                ViewParent customParent = customView.getParent();
                if (customParent != this) {
                    if (customParent != null) {
                        ((ViewGroup) customParent).removeView(customView);
                    }
                    addView(customView);
                }
        ...
        public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
            ...
            if (tabLayout != null) {
                if (this.mScrollState == 2) {
                    updateText = this.mPreviousScrollState == 1;
                } else {
                    updateText = true;
                }
                if (this.mScrollState == 2) {
                    updateIndicator = this.mPreviousScrollState != 0;
                } else {
