        ...
        this.mMenu.setCallback(new MenuBuilder.Callback() {
            public boolean onMenuItemSelected(MenuBuilder menu, MenuItem item) {
                if (NavigationView.this.mListener != null) {
                    return NavigationView.this.mListener.onNavigationItemSelected(item);
                }
                return false;
            }
    ...
    public void onMeasure(int widthSpec, int heightSpec) {
        switch (View.MeasureSpec.getMode(widthSpec)) {
            case Integer.MIN_VALUE:
                widthSpec = View.MeasureSpec.makeMeasureSpec(Math.min(View.MeasureSpec.getSize(widthSpec), this.mMaxWidth), Ints.MAX_POWER_OF_TWO);
                break;
            case 0:
                widthSpec = View.MeasureSpec.makeMeasureSpec(this.mMaxWidth, Ints.MAX_POWER_OF_TWO);
                break;
        }
