...
public class ConstraintWidget {
            ...
            if (nominator.length() > 0 && denominator.length() > 0) {
                try {
                    ...
                    if (nominatorValue > 0.0f && denominatorValue > 0.0f) {
                        dimensionRatio = dimensionRatioSide != 1 ? Math.abs(nominatorValue / denominatorValue) : Math.abs(denominatorValue / nominatorValue);
                    }
    ...
    public void connect(ConstraintAnchor.Type constraintFrom, ConstraintWidget target, ConstraintAnchor.Type constraintTo, int margin, ConstraintAnchor.Strength strength, int creator) {
        ...
        } else if (constraintTo == ConstraintAnchor.Type.CENTER) {
    ...
    public void addToSolver(LinearSystem system, int group) {
        ...
        if (this.mHorizontalResolution != 2 && (group == Integer.MAX_VALUE || (this.mLeft.mGroup == group && this.mRight.mGroup == group))) {
            if (!useHorizontalRatio || this.mLeft.mTarget == null || this.mRight.mTarget == null) {
                applyConstraints(system, wrapContent, horizontalDimensionFixed, this.mLeft, this.mRight, this.mX, this.mX + width, width, this.mMinWidth, this.mHorizontalBiasPercent, useHorizontalRatio, inHorizontalChain, this.mMatchConstraintDefaultWidth, this.mMatchConstraintMinWidth, this.mMatchConstraintMaxWidth, this.mMatchConstraintPercentWidth);
            } else {
        ...
        if (this.mVerticalResolution != 2) {
            ...
            if (this.mBaselineDistance > 0) {
                ...
                if (!useVerticalRatio || this.mTop.mTarget == null || this.mBottom.mTarget == null) {
                    applyConstraints(system, wrapContent2, verticalDimensionFixed, this.mTop, this.mBottom, this.mY, this.mY + height, height, this.mMinHeight, this.mVerticalBiasPercent, useVerticalRatio, inVerticalChain, this.mMatchConstraintDefaultHeight, this.mMatchConstraintMinHeight, this.mMatchConstraintMaxHeight, this.mMatchConstraintPercentHeight);
                } else {
    ...
    private void applyConstraints(LinearSystem system, boolean wrapContent, boolean dimensionFixed, ConstraintAnchor beginAnchor, ConstraintAnchor endAnchor, int beginPosition, int endPosition, int dimension, int minDimension, float bias, boolean useRatio, boolean inChain, int matchConstraintDefault, int matchMinDimension, int matchMaxDimension, float matchPercentDimension) {
        ...
        int dimension2 = Math.max(dimension, matchMinDimension);
        if (beginTarget == null && endTarget == null) {
            ...
            if (matchConstraintDefault == 2) {
                if (beginAnchor.getType() == ConstraintAnchor.Type.TOP || beginAnchor.getType() == ConstraintAnchor.Type.BOTTOM) {
                    percentBegin2 = system.createObjectVariable(this.mParent.getAnchor(ConstraintAnchor.Type.TOP));
                ...
                system.addConstraint(system.createRow().createRowDimensionRatio(end, begin, percentEnd2, percentBegin2, matchPercentDimension));
            } else if (wrapContent) {
                system.addConstraint(LinearSystem.createRowEquals(system, end, begin, minDimension, true));
            } else if (useRatio) {
            } else {
            ...
            if (matchConstraintDefault == 2) {
                if (beginAnchor.getType() == ConstraintAnchor.Type.TOP || beginAnchor.getType() == ConstraintAnchor.Type.BOTTOM) {
                    percentBegin3 = system.createObjectVariable(this.mParent.getAnchor(ConstraintAnchor.Type.TOP));
                    percentEnd3 = system.createObjectVariable(this.mParent.getAnchor(ConstraintAnchor.Type.BOTTOM));
                } else {
                    percentBegin3 = system.createObjectVariable(this.mParent.getAnchor(ConstraintAnchor.Type.LEFT));
                    percentEnd3 = system.createObjectVariable(this.mParent.getAnchor(ConstraintAnchor.Type.RIGHT));
                }
                system.addConstraint(system.createRow().createRowDimensionRatio(end, begin, percentEnd3, percentBegin3, matchPercentDimension));
            } else if (wrapContent) {
                system.addConstraint(LinearSystem.createRowEquals(system, end, begin, minDimension, true));
            } else if (useRatio) {
            } else {
            ...
            if (beginAnchor.getStrength() == endAnchor.getStrength()) {
                ...
                ArrayRow row2 = system.createRow();
                row2.createRowLowerThan(end, endTarget, slack2, endAnchorMargin * -1);
                system.addConstraint(row2);
            }
        } else if (useRatio) {
            ...
            if (matchConstraintDefault == 1 || matchConstraintDefault == 2) {
                if (matchConstraintDefault == 2) {
                    if (beginAnchor.getType() == ConstraintAnchor.Type.TOP || beginAnchor.getType() == ConstraintAnchor.Type.BOTTOM) {
                        percentBegin4 = system.createObjectVariable(this.mParent.getAnchor(ConstraintAnchor.Type.TOP));
                        percentEnd4 = system.createObjectVariable(this.mParent.getAnchor(ConstraintAnchor.Type.BOTTOM));
                    } else {
                        percentBegin4 = system.createObjectVariable(this.mParent.getAnchor(ConstraintAnchor.Type.LEFT));
                        percentEnd4 = system.createObjectVariable(this.mParent.getAnchor(ConstraintAnchor.Type.RIGHT));
                    }
                    system.addConstraint(system.createRow().createRowDimensionRatio(end, begin, percentEnd4, percentBegin4, matchPercentDimension));
                }
                if (matchMinDimension > dimension2) {
                    dimension2 = matchMinDimension;
                }
                if (matchMaxDimension > 0) {
                    if (matchMaxDimension >= dimension2) {
                        system.addLowerThan(end, begin, matchMaxDimension, 3);
                    } else {
                        dimension2 = matchMaxDimension;
                    }
                }
                system.addEquality(end, begin, dimension2, 3);
                ...
                if (matchMinDimension > 0) {
                    system.addGreaterThan(end, begin, matchMinDimension, 2);
                }
                system.addGreaterThan(begin, beginTarget, beginAnchorMargin, 2);
                system.addLowerThan(end, endTarget, -endAnchorMargin, 2);
                system.addCentering(begin, beginTarget, beginAnchorMargin, bias, endTarget, end, endAnchorMargin, 4);
