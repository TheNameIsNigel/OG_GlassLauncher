...
class AlertController {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (this.mScrollView != null) {
            return this.mScrollView.executeKeyEvent(event);
        }
        return false;
    }
    ...
    private void setupView() {
        ...
        setupTitle(topPanel);
        if (customPanel != null) {
            hasCustomPanel = customPanel.getVisibility() != 8;
        } else {
            hasCustomPanel = false;
        }
        if (topPanel != null) {
            hasTopPanel = topPanel.getVisibility() != 8;
        } else {
            hasTopPanel = false;
        }
        if (buttonPanel != null) {
            hasButtonPanel = buttonPanel.getVisibility() != 8;
        } else {
