...
class AppCompatTextViewAutoSizeHelper {
    ...
    private boolean suggestedSizeFitsInSpace(int suggestedSizeInPx, RectF availableSpace) {
        ...
        if ((maxLines == -1 || (layout.getLineCount() <= maxLines && layout.getLineEnd(layout.getLineCount() - 1) == text.length())) && ((float) layout.getHeight()) <= availableSpace.bottom) {
            return true;
        }
        return false;
    }
    ...
    private <T> T invokeAndReturnWithDefault(@NonNull Object object, @NonNull String methodName, @NonNull T defaultValue) {
        try {
            return getTextViewMethod(methodName).invoke(object, new Object[0]);
        } catch (Exception ex) {
