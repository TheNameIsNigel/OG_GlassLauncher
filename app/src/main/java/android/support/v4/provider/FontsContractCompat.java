                ...
                try {
                    ...
                    if (result.getStatusCode() != 0) {
                        switch (result.getStatusCode()) {
                            case 1:
                                Handler handler = callerThreadHandler;
                                ...
                                return;
                            case 2:
                                Handler handler2 = callerThreadHandler;
                                final FontRequestCallback fontRequestCallback2 = callback;
                                handler2.post(new Runnable() {
                                    public void run() {
                                        fontRequestCallback2.onTypefaceRequestFailed(-3);
                                    }
                                });
                                return;
                            default:
                                Handler handler3 = callerThreadHandler;
                                final FontRequestCallback fontRequestCallback3 = callback;
                                handler3.post(new Runnable() {
                                    public void run() {
                                        fontRequestCallback3.onTypefaceRequestFailed(-3);
                                    }
                                });
                                return;
                        }
                        ...
                        if (fonts == null || fonts.length == 0) {
                            Handler handler4 = callerThreadHandler;
                            final FontRequestCallback fontRequestCallback4 = callback;
                            handler4.post(new Runnable() {
                                public void run() {
                                    fontRequestCallback4.onTypefaceRequestFailed(1);
                                }
                            });
                            return;
                        }
                        for (FontInfo font : fonts) {
                            if (font.getResultCode() != 0) {
                                ...
                                if (resultCode < 0) {
                                    Handler handler5 = callerThreadHandler;
                                    final FontRequestCallback fontRequestCallback5 = callback;
                                    handler5.post(new Runnable() {
                                        public void run() {
                                            fontRequestCallback5.onTypefaceRequestFailed(-3);
                                        }
                                    });
                                    return;
                                }
                                Handler handler6 = callerThreadHandler;
                                final FontRequestCallback fontRequestCallback6 = callback;
                                handler6.post(new Runnable() {
                                    public void run() {
                                        fontRequestCallback6.onTypefaceRequestFailed(resultCode);
                                    }
                                });
                                return;
                            }
                        ...
                        if (typeface == null) {
                            Handler handler7 = callerThreadHandler;
                            final FontRequestCallback fontRequestCallback7 = callback;
                            handler7.post(new Runnable() {
                                public void run() {
                                    fontRequestCallback7.onTypefaceRequestFailed(-3);
                                }
                            });
                            return;
                        }
                        Handler handler8 = callerThreadHandler;
                        final FontRequestCallback fontRequestCallback8 = callback;
                        handler8.post(new Runnable() {
                            public void run() {
                                fontRequestCallback8.onTypefaceRetrieved(typeface);
                            }
                        });
                    }
                } catch (PackageManager.NameNotFoundException e) {
                    Handler handler9 = callerThreadHandler;
                    final FontRequestCallback fontRequestCallback9 = callback;
                    handler9.post(new Runnable() {
                        public void run() {
                            fontRequestCallback9.onTypefaceRequestFailed(-1);
                        }
                    });
                }
            }
