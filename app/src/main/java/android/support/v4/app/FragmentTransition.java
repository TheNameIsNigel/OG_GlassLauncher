...
class FragmentTransition {
    ...
    private static Object configureSharedElementsOrdered(FragmentTransitionImpl impl, ViewGroup sceneRoot, View nonExistentView, ArrayMap<String, String> nameOverrides, FragmentContainerTransition fragments, ArrayList<View> sharedElementsOut, ArrayList<View> sharedElementsIn, Object enterTransition, Object exitTransition) {
        ...
        OneShotPreDrawListener.add(sceneRoot, new Runnable() {
            public void run() {
                ArrayMap<String, View> inSharedElements = FragmentTransition.captureInSharedElements(fragmentTransitionImpl, arrayMap, finalSharedElementTransition, fragmentContainerTransition);
                if (inSharedElements != null) {
                ...
                if (finalSharedElementTransition != null) {
                    fragmentTransitionImpl.swapSharedElementTargets(finalSharedElementTransition, arrayList2, arrayList);
            ...
            switch (isPop ? INVERSE_OPS[op.cmd] : op.cmd) {
                ...
                case 7:
                    if (isReorderedTransaction) {
                        boolean setLastIn2 = fragment.mIsNewlyAdded;
                    } else {
                        setLastIn2 = !fragment.mAdded ? !fragment.mHidden : false;
                    }
                    wasAdded = true;
                    break;
                case 3:
                case 6:
                    if (isReorderedTransaction) {
                        boolean setFirstOut2 = (fragment.mAdded || fragment.mView == null || fragment.mView.getVisibility() != 0) ? false : fragment.mPostponedAlpha >= 0.0f;
                    } else {
                        setFirstOut2 = fragment.mAdded ? !fragment.mHidden : false;
                    }
                    wasRemoved = true;
                    break;
                case 4:
                    if (isReorderedTransaction) {
                        setFirstOut = (!fragment.mHiddenChanged || !fragment.mAdded) ? false : fragment.mHidden;
                    } else {
                        setFirstOut = fragment.mAdded ? !fragment.mHidden : false;
                    }
                    wasRemoved = true;
                    break;
                case 5:
                    if (isReorderedTransaction) {
                        setLastIn = (!fragment.mHiddenChanged || !(fragment.mHidden ^ true)) ? false : fragment.mAdded;
                    } else {
