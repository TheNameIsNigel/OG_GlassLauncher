...
public class LifecycleRegistry extends Lifecycle {
    ...
    private Lifecycle.State calculateTargetState(LifecycleObserver observer) {
        ...
        return min(min(this.mState, previous == null ? null : previous.getValue().mState), this.mParentStates.isEmpty() ? null : this.mParentStates.get(this.mParentStates.size() - 1));
    }

    public void addObserver(LifecycleObserver observer) {
        boolean isReentrance = false;
        ObserverWithState statefulObserver = new ObserverWithState(observer, this.mState != Lifecycle.State.DESTROYED ? Lifecycle.State.INITIALIZED : Lifecycle.State.DESTROYED);
